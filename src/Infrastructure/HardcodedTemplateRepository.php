...
class HardcodedTemplateRepository implements TemplateRepositoryInterface
{
    ...
    {
        $this->queries = [];
        $this->queries[] = 'SELECT MONTH(DATA_ZDARZ) AS lp, MONTHNAME(DATA_ZDARZ) AS miesiac, COUNT(*) AS zdarzenia FROM zdarzenie %zdarzenie_filter% GROUP BY lp,miesiac ORDER BY lp ASC;';
        $this->queries[] = 'SELECT DAYOFWEEK(DATA_ZDARZ) AS lp, DAYNAME(DATA_ZDARZ) AS dzien_tygodnia, COUNT(*) AS zdarzenia FROM zdarzenie %zdarzenie_filter% GROUP BY lp,dzien_tygodnia ORDER BY lp ASC;';
        $this->queries[] = 'SELECT HOUR(GODZINA_ZDARZ) AS godzina, COUNT(*) AS zdarzenia FROM zdarzenie  %zdarzenie_filter%  GROUP BY godzina;';
  ...
  INNER JOIN geod ON geod.kod=zdarzenie.geod_kod ORDER BY zdarzenia DESC;';
    ...
    public function getAll(): array
    {
        ...
        foreach ($this->queries as $sqlQuery) {
            $queries[] = new QueryTemplate($sqlQuery);
        }
        return $queries;
    }
}