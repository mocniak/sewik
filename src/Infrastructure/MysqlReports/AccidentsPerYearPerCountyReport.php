...
class AccidentsPerYearPerCountyReport
{
    ...
    {
        $counties = $this->database->execute('SELECT DISTINCT POWIAT FROM accident;');
        ...
        foreach ($counties as $countyRow) {
            ...
            $accidentsByCounties[$countyName] = $this->database->execute(
                'SELECT YEAR(DATA_ZDARZ) as rok, COUNT(1) as zdarzenia FROM accident WHERE POWIAT = \'' . $countyName . '\' GROUP BY rok;'
            );
        }
        $result = [];
